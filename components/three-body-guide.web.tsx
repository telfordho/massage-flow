...
function SurfacePatch({
  ...
  if (region === "FOREARM_PALM") {
    return (
      <>
        <mesh renderOrder={2} position={[side === "LEFT" ? -0.9 : 0.9, 0.05, -0.02]} rotation={[0, 0, side === "LEFT" ? -0.2 : 0.2]} scale={[1.05, 1.05, 1.05]} onClick={click}>
          <capsuleGeometry args={[0.145, 0.72, 10, 18]} />
...
function WebMannequin({
  ...
  return (
    <group rotation={[0, rotation, 0]} scale={[zoom, zoom, zoom]}>
      ...
      <mesh position={[0.9, 0.56, -0.02]} rotation={[0, 0, 0.2]}><capsuleGeometry args={[0.14, 1.25, 10, 18]} /><meshStandardMaterial color={BODY} roughness={0.8} /></mesh>
      <mesh position={[-1.06, -0.37, -0.02]}><sphereGeometry args={[0.24, 20, 20]} /><meshStandardMaterial color={BODY} roughness={0.8} /></mesh>
      <mesh position={[1.06, -0.37, -0.02]}><sphereGeometry args={[0.24, 20, 20]} /><meshStandardMaterial color={BODY} roughness={0.8} /></mesh>
